...
class CheckpointCallback(TrainerCallback):
    ...
    def _cleanup_kaggle_versions(self, max_keep=3):
        """Delete old Kaggle model versions, keep only the latest max_keep."""
        try:
            import subprocess, json
            # List versions using kaggle CLI
            result = subprocess.run(
                ["kaggle", "models", "instances", "versions",
                 KAGGLE_USERNAME, "gpt-oss-120b-aimo3-sft-v3",
            ...
            if result.returncode != 0:
                ...
                api = KaggleApi()
                api.authenticate()
                versions = api.model_instance_versions(
                    KAGGLE_USERNAME, "gpt-oss-120b-aimo3-sft-v3",
                    "transformers", "default"
                )
                if hasattr(versions, '__len__') and len(versions) > max_keep:
                    # Versions are sorted newest first — delete the oldest
                    to_delete = versions[max_keep:]
                    for v in to_delete:
                        ver_num = v.versionNumber
                        try:
                            api.model_instance_version_delete(
                                KAGGLE_USERNAME, "gpt-oss-120b-aimo3-sft-v3",
                                "transformers", "default", ver_num
                            )
                            print(f"  🗑  Deleted Kaggle version {ver_num}")
                        except Exception as e:
                            print(f"  ⚠️  Could not delete version {ver_num}: {e}")
                else:
                    print(f"  ℹ️  {len(versions) if hasattr(versions, '__len__') else '?'} versions — no cleanup needed")
        except Exception as e:
            print(f"  ⚠️  Version cleanup skipped: {e}")

    def _safe_merge_and_upload(self, state, is_final=False):
        """
        Merge LoRA → 4bit → upload, WITHOUT corrupting training state.
        
        Strategy:
          1. Save LoRA weights to disk
          2. Merge into base and save as 4bit
          3. Reload LoRA weights back onto model (restores training state)
        """
        step = state.global_step
        loss = self._get_loss(state)
        tag = "FINAL" if is_final else f"Step {step}"

        print(f"\n  [{tag}] loss={loss} — safe merge starting...")

        lora_backup_dir = f"_lora_backup_step_{step}"
        merged_dir = f"merged-4bit-step-{step}"

        try:
            # Step 1: Save LoRA weights (backup before merge corrupts them)
            print(f"  1/4 Saving LoRA backup...")
            model.save_pretrained(lora_backup_dir)
            tokenizer.save_pretrained(lora_backup_dir)

            # Step 2: Merge and save as 4bit
            print(f"  2/4 Merging to 4bit...")
            t_merge = time.time()
            ...
            # Step 3: Restore LoRA weights (undo merge corruption)
            if not is_final:
                print(f"  3/4 Restoring LoRA weights...")
                from peft import PeftModel
                model.load_adapter(lora_backup_dir, adapter_name="default")
                print(f"  ✅ LoRA restored — training state intact")
            else:
                print(f"  3/4 Final merge — no restore needed")

            # Step 4: Upload to Kaggle
            print(f"  4/4 Uploading to Kaggle...")
            try:
                import kagglehub
                ...
                kagglehub.model_upload(
                    handle, merged_dir,
                    version_notes=f"{'FINAL ' if is_final else ''}merged 4bit | step={step} | loss={loss}",
                    license_name="Apache 2.0",
                )
                upload_mins = (time.time() - t_upload) / 60
                print(f"  ✅ Upload done in {upload_mins:.1f} mins")

                # Delete old versions — keep only latest 3
                self._cleanup_kaggle_versions(max_keep=3)
            ...
            # Try to restore LoRA even if merge failed
            if os.path.exists(lora_backup_dir) and not is_final:
                try:
                    from peft import PeftModel
                    model.load_adapter(lora_backup_dir, adapter_name="default")
                    print(f"  ✅ LoRA restored after failure — training can continue")
                except Exception as e2:
                    print(f"  ❌ CRITICAL: Could not restore LoRA: {e2}")
                    print(f"  Training state may be corrupted!")

        finally:
            # Cleanup
            for d in [lora_backup_dir, merged_dir]:
                if os.path.exists(d):
                    try:
                        shutil.rmtree(d)
                        print(f"  🗑  Freed: {d}")
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        # Early merge + upload at step 100
        if step == 100 and self.last_upload_step < 100:
            self.last_upload_step = step
            control.should_save = True
            self._safe_merge_and_upload(state)

        # Merge + upload every 500 steps
        if step > 0 and step % MERGE_UPLOAD_EVERY == 0 and step != self.last_upload_step:
            self.last_upload_step = step
            self._safe_merge_and_upload(state)

    def on_train_end(self, args, state, control, **kwargs):
        self._safe_merge_and_upload(state, is_final=True)
...
trainer = SFTTrainer(
    ...
    args=SFTConfig(
        ...
        save_strategy="steps",
        save_steps=500,
        save_total_limit=2,              # keep last 2 — enough for resume
        logging_steps=1,
        eval_strategy="no",
        max_grad_norm=0.5,
        report_to=REPORT_TO,
...
t0 = time.time()
import glob
checkpoints = sorted(glob.glob("sft-output-120b-v3/checkpoint-*"))
if checkpoints:
