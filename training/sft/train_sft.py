...
"""
    ...
    def _quick_eval(self, step, n=50):
        ...
        for i in range(min(n, len(raw_eval))):
            try:
                ...
                # Don't use enable_thinking — model already learned <think> from SFT
                ...
                # Debug: print first 3 eval results
                if i < 3:
                    has_boxed = 'boxed' in text if text else False
                    has_think = '<think>' in text if text else False
                    print(f"      [{i}] pred={pred} gt={gt} boxed={has_boxed} think={has_think} len={len(text) if text else 0}")
                    print(f"      last 200: {text[-200:] if text else 'EMPTY'}")
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if step > 0 and step % self.upload_every == 0 and step != self.last_step:
            ...
            # Upload LoRA only (NOT merged — merging causes OOM)
            try:
                import kagglehub
                self._write_readme(ckpt, step, loss, score)
                handle = f"{KAGGLE_USERNAME}/qwen3-32b-aimo3-sft-lora/transformers/default"
                kagglehub.model_upload(
                    handle, ckpt,
                    version_notes=f"LoRA step {step} | loss={loss} | acc={score:.3f}",
                    license_name="Apache 2.0",
                )
                print(f"    LoRA uploaded: {handle} (step {step})")
            except Exception as e:
    ...
    def on_train_end(self, args, state, control, **kwargs):
        ...
        # Save LoRA + trainer state (merge happens AFTER trainer is deleted)
        model.save_pretrained("checkpoint-sft-final")
        state.save_to_json("checkpoint-sft-final/trainer_state.json")
        self._write_readme("checkpoint-sft-final", step, loss, score)
        print(f"    Final LoRA saved to checkpoint-sft-final/")
...
trainer = SFTTrainer(
    ...
    args=SFTConfig(
        ...
        save_steps=300,
        logging_steps=1,
        eval_strategy="no",            # DISABLED — our callback does eval instead
        max_grad_norm=1.0,
        report_to=REPORT_TO,
...
# Delete trainer FIRST to free memory
del trainer
gc.collect()
import torch
torch.cuda.empty_cache()
print(f"  Trainer deleted. Freed GPU memory.")

# NOW merge and upload — enough memory available
print(f"\n  Merging LoRA → full 16-bit model...")
try:
    import kagglehub
    ...
    # Write README
    readme = f"""# Qwen3-32B AIMO3 — SFT Final

## Model
- **Base**: Qwen3-32B
- **Training**: SFT on 120K Nemotron Math v2 gold solutions (1 epoch)
- **Format**: Reasoning inside `<think>` tags, answer in `\\boxed{{}}`
...
## Usage
Use with vLLM for inference in the AIMO3 submission notebook.
"""
    with open(os.path.join(merged, "README.md"), "w") as f:
        f.write(readme)
    ...
    kagglehub.model_upload(handle, merged,
        version_notes=f"FINAL merged | {train_time/3600:.1f}h training",
        license_name="Apache 2.0")
    print(f"  Merged model uploaded!")
except Exception as e:
    print(f"  Merge/upload failed: {e}")
    print(f"  LoRA checkpoint saved at: checkpoint-sft-final/")
...
print(f"\n  Kaggle models:")
print(f"    LoRA: {KAGGLE_USERNAME}/qwen3-32b-aimo3-sft-lora")
print(f"    Merged: {KAGGLE_USERNAME}/qwen3-32b-aimo3-sft")
print(f"\n  Next: Run critique training")
print("=" * 60)
