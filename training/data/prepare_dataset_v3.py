#!/usr/bin/env python3
"""
AIMO3: Prepare SFT Dataset v3 — CORRECTED
==========================================
Filters nvidia/Nemotron-Math-v2 for the highest-quality training signal:

  - Splits: high_part00, high_part01 ONLY (high reasoning depth trajectories)
    (high_part02 is 100% StackExchange → skipped)
  - Source: data_source == 'aops' ONLY (competition math, not StackExchange)
  - Difficulty: reason_high_with_tool accuracy < 0.5
    (problems the model fails even at max reasoning + tools = competition failures)
  - Trajectories: with-tool only (matches competition inference)
  - Per problem: shortest trajectory (less noise, faster inference)

Saves locally to ./data/nemotron-sft-v3/ and uploads to Kaggle.

Usage:
    python3.12 prepare_dataset_v3.py
    python3.12 prepare_dataset_v3.py --splits high_part00  # single split test
    python3.12 prepare_dataset_v3.py --no-upload           # skip Kaggle upload
    python3.12 prepare_dataset_v3.py --output-dir ./data/my-run
...
parser.add_argument("--splits", nargs="+",
                    default=["high_part00", "high_part01"],
                    help="Splits to process (default: high_part00 high_part01)")
...
DATA_SOURCE     = "aops"                     # Competition math only, skip StackExchange
SEED            = 42
...
def is_aops(example):
    return example.get("data_source") == DATA_SOURCE
...
print(f"  Source:  {DATA_SOURCE} only")
...
for split_name in args.splits:
    ...
    # Filter 1: AoPS only
    before = len(ds)
    ds = ds.filter(is_aops, num_proc=4)
    print(f"  AoPS filter:  {before:,} → {len(ds):,}")

    # Filter 2: with-tool only
    before = len(ds)
    ds = ds.filter(has_tools, num_proc=4)
    print(f"  Tool filter:  {before:,} → {len(ds):,}")

    # Filter 3: hard (reason_high_with_tool < 0.5)
    before = len(ds)
...
# HF dataset (native format for easy loading)
...
readme = f"""# Nemotron-Math-v2 SFT v3 — AoPS Hard, High Reasoning

## What This Is
Competition-grade SFT dataset for AIMO3. Filtered from `nvidia/Nemotron-Math-v2`.

## Filters Applied
| Filter | Value | Rationale |
|--------|-------|-----------|
| Source | `aops` only | AoPS = competition math (what AIMO tests) |
| Splits | `high_part00`, `high_part01` | High reasoning depth trajectories only |
...
## Source
`nvidia/Nemotron-Math-v2` (CC BY 4.0)
"""
...
if not args.no_upload:
    ...
    if not (KAGGLE_TOKEN and KAGGLE_USER):
        ...
        try:
            ...
            # Prepare upload dir (parquet + readme only)
            ...
            kagglehub.dataset_upload(
                handle, upload_dir,
                version_notes=(
                    f"v3: AoPS-only, reason_high_with_tool<{MAX_ACC}, "
                    f"high_part00+01, {len(final_df):,} rows"
                ),
            )
...
for i, (_, row) in enumerate(sample.iterrows()):
    ...
    print(f"\n  ── Sample {i+1} (acc={row['accuracy']:.3f}) ──")
